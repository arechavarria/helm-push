...
func TestSetVersion(t *testing.T) {
	...
	c.SetVersion("latest")
	if c.V3.Metadata.Version != "latest" {
		t.Errorf("expected chart version to be latest, instead got %s", c.V3.Metadata.Version)
	}
}
...
func TestGetChartByName(t *testing.T) {
	...
	if c.V3.Metadata.Name != "mychart" {
		t.Errorf("expexted chart name to be mychart, instead got %s", c.V3.Metadata.Name)
	}
	if c.V3.Metadata.Version != "0.1.0" {
		t.Errorf("expexted chart version to be 0.1.0, instead got %s", c.V3.Metadata.Version)
	}
}
